...
onBeforeMount(() => {
  promotionStore.getPromotionProgrammes
})
...
<template>
  <div class="row row-cols-1 row-cols-md-3  g-2">
        <div class="col" v-for="(offre, index) in promotionStore.offresProgrammes" :key="index">
          <div
            class="card border-0"
          ...
          >
            ...
            >
              <router-link
                :to="`/detail_reservation_ticket/${encryptParam(offre.companieInfos.uid)}/${encryptParam(offre.uid)}`"
                style="
                  border: 1px solid;
                  border-radius: 5px;
                  border-color: #a6a6a6;
              ...
              <button class="btn btn-primary" id="badges0121">
                {{ offre.lieu_depart }} - {{ offre.destination }}
              </button>
            </div>
          </div>
        </div>
